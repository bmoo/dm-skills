...
- **Log the pass.** Through the checker's `findings_log` module:
  one `log_run(<the producing skill>, <the criteria check ids graded>,
  tier="judgement", verdict=<the verdict>)`, and one
  `log_finding(<the producing skill>, <check id>, tier="judgement",
