...
# --------------------------------------------------------------------------- #
# node-map — the counterfactual mode.
# --------------------------------------------------------------------------- #
#
# node-map draws as-built by default and these checks say nothing about that
# view. They are gated on the **strike ledger**, the table the as-if mode files
# its reading of the clue web into
# (`node-map/SKILL.md` — "File the reading as a **strike ledger**").
# No ledger, no findings: an as-built map is not a counterfactual missing its
# ledger, it is a different output, and the ruling that put this mode here also
# ruled the everyday view must not get more expensive to serve a rare one.
#
# The names/locates distinction the ledger carries is a record-model ruling
# settled before this mode (`docs/clue-edges-names-vs-locates.md`): the cut is
# binary, it belongs to the edge rather than the clue, and a sentence that does
# not say does not count as locating.

_STRIKE_LEDGER_ARROW = "→"
_LEDGER_TYPES = ("locates", "names")
_LEDGER_STATES = ("struck", "standing")

_STRUCK_EDGE_GLYPH = "╳"
_ORPHANED_RULE_GLYPH = "╎"

_ORPHAN_VERDICT_RES = {
    "as-built": re.compile(r"^\s*Orphaned as-built\s*:\s*(?P<value>.+?)\s*$", re.MULTILINE | re.IGNORECASE),
    "as-if": re.compile(r"^\s*Orphaned as-if\s*:\s*(?P<value>.+?)\s*$", re.MULTILINE | re.IGNORECASE),
}
...
def _strike_ledger(artifact: str) -> List[_LedgerRow]:
    """Every strike-ledger row in the artifact, in order.

    A ledger row is a four-cell markdown table row whose first cell carries the
    ``→`` that names the edge — ``| A → B | locates | "…" | struck |``. Rows are
    picked out by that shape rather than by locating the table's header, so a
    ledger split across two tables (one per source node, say) still reads as one
    ledger; the header row and the ``|---|`` rule carry no arrow and are skipped
    for free. A row whose type or state is off-list is still returned — reporting
    it is the typing check's job, and dropping it here would make that check
    silent on exactly the artifact it is for."""
    rows: List[_LedgerRow] = []
    for number, line in enumerate(artifact.splitlines(), 1):
        stripped = line.strip()
        if not stripped.startswith("|") or _STRIKE_LEDGER_ARROW not in stripped:
            continue
        cells = [cell.strip() for cell in stripped.strip("|").split("|")]
        if len(cells) != 4 or _STRIKE_LEDGER_ARROW not in cells[0]:
            continue
        source, _, target = cells[0].partition(_STRIKE_LEDGER_ARROW)
        rows.append(
            _LedgerRow(
                line=number,
                source=source.strip(),
                target=target.strip(),
                edge_type=cells[1].strip().strip("`").lower(),
                read_from=cells[2].strip(),
                state=cells[3].strip().strip("`").lower(),
            )
        )
    return rows


def _reachable(rows: List[_LedgerRow], drop_struck: bool) -> set:
    """The nodes a party can actually get to, walking `locates` edges only.

    Roots are the ledger's own sources that are never anybody's target — where
    the party already are. They are computed over **every** row, struck ones
    included, so striking the only edge into a node cannot promote that node to a
    root and quietly re-reach it."""
    nodes = {row.source for row in rows} | {row.target for row in rows}
    targets = {row.target for row in rows}
    live = [
        row
        for row in rows
        if row.edge_type == "locates" and not (drop_struck and row.state == "struck")
    ]
    adjacency: Dict[str, List[str]] = {}
    for row in live:
        adjacency.setdefault(row.source, []).append(row.target)
    queue = [node for node in nodes if node not in targets]
    seen = set(queue)
    while queue:
        node = queue.pop(0)
...
def _declared_orphans(artifact: str, which: str) -> Optional[set]:
    """The node set one ``Orphaned …:`` verdict line declares, or None when the
    line is absent. ``none`` is the empty set — an answer, not a missing line.

    The skill specifies ``·`` between names; a comma is accepted too, so that a
    page writing the more natural English separator gets a true verdict on its
    arithmetic instead of an arithmetic finding about its punctuation."""
    match = _ORPHAN_VERDICT_RES[which].search(artifact)
    if match is None:
        return None
...
def _named(nodes: set) -> str:
    return " · ".join(sorted(nodes)) if nodes else "none"


@register_check("node-map/lead-edges-typed-names-or-locates", "node-map")
def check_lead_edges_typed(artifact: str) -> List[Finding]:
    """Every strike-ledger edge is typed from the closed two-value vocabulary —
    `locates` or `names` — with the words it was read off recorded beside it
    (`node-map/SKILL.md` — "Type every lead edge in scope — `locates` or
    `names`", "Two values, never a third").

    The two-value cut is the ruling, not a simplification: a prospective locator,
    a stale sighting and a locator sitting somewhere unreachable all answer the
    counterfactual's only question the same way, so a third value would be
    decoration (`docs/clue-edges-names-vs-locates.md` — "The cut is binary"). An
    off-list type is therefore the defect this fires on, not a richer reading.

    The `Read from` cell is checked for presence only. It is what makes the
    degrade honest — the reading is stated rather than silently assumed — and
    whether the words quoted really carry the type is judgement, not arithmetic.
    """
    location = "the strike ledger"
    rows = _strike_ledger(artifact)
    findings: List[Finding] = []
    for row in rows:
        edge = f"{row.source} → {row.target}"
        if row.edge_type not in _LEDGER_TYPES:
            findings.append(
                Finding(
                    check_id="node-map/lead-edges-typed-names-or-locates",
                    expected="every ledger edge typed `locates` or `names` — two values, never a third",
                    actual=f"{edge} is typed {row.edge_type or '(blank)'!r}",
                    output_location=location,
                )
            )
        if row.state not in _LEDGER_STATES:
            findings.append(
                Finding(
                    check_id="node-map/lead-edges-typed-names-or-locates",
                    expected="every ledger edge marked `struck` or `standing` after the strike",
                    actual=f"{edge} is marked {row.state or '(blank)'!r}",
                    output_location=location,
                )
            )
        if not row.read_from:
            findings.append(
                Finding(
                    check_id="node-map/lead-edges-typed-names-or-locates",
                    expected="every ledger edge records the words its type was read off",
                    actual=f"{edge} states no reading",
                    output_location=location,
                )
            )
    return findings
...
@register_check("node-map/struck-edge-drawn-not-deleted", "node-map")
def check_struck_edge_drawn(artifact: str) -> List[Finding]:
    """The struck clue is drawn over with the struck-edge glyph, never removed
    from the as-if view (`node-map/SKILL.md` — "a STRUCK edge — the clue the
    counterfactual removes. Drawn over, never deleted").

    A ledger that strikes a row and a drawing with no strike glyph in it is the
    failure: a gap where a lead used to be reads as a web that never had it, and
    the DM cannot tell the counterfactual from the record.

    Presence, deliberately, not one glyph per struck row. The as-if view's own
    legend defines the glyph, so a count cannot separate *two strikes drawn* from
    *one drawn and one deleted* — and a count that fires on the legend would push
    pages to drop the legend, which is worse. The narrower assertion is the one
    that never lies about what it saw."""
    location = "the as-if diagram"
    rows = _strike_ledger(artifact)
    if not rows:
        return []
    struck = [row for row in rows if row.state == "struck"]
    if not struck:
        return []
    if _STRUCK_EDGE_GLYPH in artifact:
        return []
    return [
        Finding(
            check_id="node-map/struck-edge-drawn-not-deleted",
            expected=f"the struck edge drawn over with '{_STRUCK_EDGE_GLYPH}', not removed",
            actual="no struck-edge glyph anywhere, but the ledger strikes "
            + " · ".join(f"{row.source} → {row.target}" for row in struck),
            output_location=location,
        )
    ]


@register_check("node-map/orphaned-node-dashed-box", "node-map")
def check_orphaned_node_dashed_box(artifact: str) -> List[Finding]:
    """Every node the as-if verdict calls orphaned is drawn with the dashed rule
    (`node-map/SKILL.md` — "an ORPHANED node — no locating edge reaches it any
    more", "dashed rule replaces whatever rule the box had").

    The override half is the point and it is why this reads the rule rather than
    the word ORPHANED: a hub or a destination node keeps its double rule under
    every other styling rule in the vocabulary, so an orphaned hub drawn ``║ … ║``
    with the word ORPHANED inside is legible only to a reader already looking for
    it. Dashed wins, so the DM's eye finds the unreachable nodes without reading.

    The name must sit **between** the line's first and last dashed rule — inside
    the box, not in the prose hanging off the end of somebody else's. That is the
    difference between a node drawn orphaned and a node mentioned beside one that
    was. What the check cannot tell apart is a real box from a caption dressed in
    the same rule; it reads the glyph, and the row says so.

    Matched case-insensitively, because boxes are drawn in caps. It grades what
    the verdict names — a node absent from both is the orphan-set check's
    finding, not this one's."""
    location = "the as-if diagram"
    rows = _strike_ledger(artifact)
    if not rows:
        return []
    declared = _declared_orphans(artifact, "as-if")
    if not declared:
        return []
    dashed = [
        line[
            line.index(_ORPHANED_RULE_GLYPH) : line.rindex(_ORPHANED_RULE_GLYPH)
        ].lower()
        for line in artifact.splitlines()
        if line.count(_ORPHANED_RULE_GLYPH) >= 2
    ]
    findings: List[Finding] = []
    for node in sorted(declared):
        if not any(node.lower() in line for line in dashed):
            findings.append(
                Finding(
                    check_id="node-map/orphaned-node-dashed-box",
                    expected=f"{node} drawn with the dashed rule '{_ORPHANED_RULE_GLYPH}'",
                    actual=f"{node} is called orphaned by the verdict but no box line of "
                    "its own carries the dashed rule",
                    output_location=location,
                )
            )
    return findings
...
@register_check("node-map/orphan-sets-recomputed", "node-map")
def check_orphan_sets_recomputed(artifact: str) -> List[Finding]:
    """Both orphan sets are reported and each equals reachability recomputed over
    the ledger (`node-map/SKILL.md` — "Recompute reachability **twice** over the
    ledger", "Both are mandatory").

    This is the mode's numeric self-verification, and it is here because the
    mode's output *is* a count: a prototype control arm that skipped verifying
    its own arithmetic shipped four modelling bugs and one nearly produced a
    false headline. The as-built pass is the half that stops precisely that
    headline — *this strike orphans both destination nodes* is a different claim
    when both were already unreachable, and only computing the two sets
    separately can tell them apart.

    Reachability walks `locates` edges only, which is the whole reason the
    names/locates cut was settled first: read untyped, the Lost Vein strike
    leaves four edges standing at one node and reports no orphan at all."""
    location = "the Orphaned as-built / Orphaned as-if verdict lines"
    rows = _strike_ledger(artifact)
    if not rows:
        return []
    nodes = {row.source for row in rows} | {row.target for row in rows}
    findings: List[Finding] = []
    for which, drop_struck in (("as-built", False), ("as-if", True)):
        declared = _declared_orphans(artifact, which)
        computed = nodes - _reachable(rows, drop_struck=drop_struck)
        if declared is None:
            findings.append(
                Finding(
                    check_id="node-map/orphan-sets-recomputed",
                    expected=f"an `Orphaned {which}:` line — both sets are mandatory",
                    actual=f"no `Orphaned {which}:` line; the ledger computes {_named(computed)}",
                    output_location=location,
                )
            )
            continue
        if declared != computed:
            findings.append(
                Finding(
                    check_id="node-map/orphan-sets-recomputed",
                    expected=f"Orphaned {which}: {_named(computed)} — recomputed from the ledger",
                    actual=f"the page reports {_named(declared)}",
                    output_location=location,
                )
            )
    return findings
...
def _hours(text: str) -> set:
    ...
    punctuation rather than about the contract — the same call
    `node-map/orphan-sets-recomputed` makes when it accepts a comma for a `·`."""
    found = set()
