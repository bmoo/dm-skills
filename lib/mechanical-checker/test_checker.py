...
# --------------------------------------------------------------------------- #
#
# The two headline fixtures are read out of the frozen ablation scaffolds: the
# Lost Vein web, where striking Rosalyn Briarwood's one lead leaves both
# destination nodes and Emberwillow with no locating route, and the Wild Sheep
# Chase web, where the Oakhouse carries three locating leads and striking the
# loudest of them changes nothing.

NODE_MAP_TYPED = "node-map/lead-edges-typed-names-or-locates"
NODE_MAP_STRUCK = "node-map/struck-edge-drawn-not-deleted"
NODE_MAP_DASHED = "node-map/orphaned-node-dashed-box"
NODE_MAP_ORPHANS = "node-map/orphan-sets-recomputed"
NODE_MAP_ALL = [NODE_MAP_TYPED, NODE_MAP_STRUCK, NODE_MAP_DASHED, NODE_MAP_ORPHANS]

ASIF_ORPHANS = "node_map_asif_orphans.md"
ASIF_NO_ORPHANS = "node_map_asif_no_orphans.md"


def test_strike_that_orphans_passes_every_row():
    assert run_checks(_load(ASIF_ORPHANS), "node-map", NODE_MAP_ALL) == []


def test_strike_that_orphans_nothing_passes_every_row():
    assert run_checks(_load(ASIF_NO_ORPHANS), "node-map", NODE_MAP_ALL) == []


def test_as_built_map_is_ungraded_by_every_counterfactual_row():
    # The everyday view carries no strike ledger, so none of these rows has
    # anything to say about it — the as-built path is exactly as expensive as it
    # was before the mode existed.
    assert run_checks(_load("node_map_asbuilt_only.md"), "node-map", NODE_MAP_ALL) == []


def test_as_built_orphan_line_catches_the_false_headline():
    # The as-if set is right; the as-built line claims none, which credits the
    # strike with two orphans that were already orphans.
    findings = run_checks(_load("node_map_asif_false_headline.md"), "node-map", NODE_MAP_ALL)
    assert len(findings) == 1
    assert findings[0].check_id == NODE_MAP_ORPHANS
    assert "as-built" in findings[0].expected
    assert "Redfang Castle" in findings[0].expected
    assert findings[0].actual == "the page reports none"


def test_missing_orphan_line_is_its_own_finding():
    artifact = _load(ASIF_ORPHANS).replace(
        "Orphaned as-built: Redfang Castle · Halgrim Deepdelver · Stonesong Cave\n", ""
    )
    findings = run_checks(artifact, "node-map", [NODE_MAP_ORPHANS])
    assert len(findings) == 1
    assert "no `Orphaned as-built:` line" in findings[0].actual


def test_untyped_read_loses_both_destination_nodes():
    # Why the names/locates cut had to be settled before the mode: read untyped,
    # the same strike leaves four edges standing at Redfang Castle and one at
    # Stonesong Cave and reports neither. Only Emberwillow, whose single inbound
    # edge is the struck one, survives an untyped read as an orphan.
    untyped = _load(ASIF_ORPHANS).replace("| names |", "| locates |")
    findings = run_checks(untyped, "node-map", [NODE_MAP_ORPHANS])
    assert [f.expected for f in findings] == [
        "Orphaned as-built: none — recomputed from the ledger",
        "Orphaned as-if: Ruins of Emberwillow — recomputed from the ledger",
    ]
    for finding in findings:
        assert "Redfang Castle" not in finding.expected
        assert "Stonesong Cave" not in finding.expected


def test_third_edge_type_is_rejected():
    findings = run_checks(_load("node_map_asif_untyped_edge.md"), "node-map", NODE_MAP_ALL)
    assert len(findings) == 1
    assert findings[0].check_id == NODE_MAP_TYPED
    assert "prospective" in findings[0].actual


def test_edge_with_no_stated_reading_is_a_finding():
    artifact = _load(ASIF_NO_ORPHANS).replace(
        '| Guz → Ahmed Noke | names | "he speaks of Noke as *Master Noke*, always" | standing |',
        "| Guz → Ahmed Noke | names |  | standing |",
    )
    findings = run_checks(artifact, "node-map", [NODE_MAP_TYPED])
    assert len(findings) == 1
    assert findings[0].actual == "Guz → Ahmed Noke states no reading"


def test_deleted_edge_is_caught_where_a_struck_one_is_not():
    findings = run_checks(_load("node_map_asif_edge_deleted.md"), "node-map", NODE_MAP_ALL)
    assert len(findings) == 1
    assert findings[0].check_id == NODE_MAP_STRUCK
    assert "Copperton → Ruins of Emberwillow" in findings[0].actual


def test_a_ledger_that_strikes_nothing_is_not_asked_for_a_glyph():
    # The as-built ledger of a "what does this web look like typed" ask carries
    # no struck row, so the strike glyph is not owed.
    artifact = _load(ASIF_NO_ORPHANS).replace("| struck |", "| standing |")
    assert "╳" in artifact  # the drawing still legends it
    assert run_checks(artifact, "node-map", [NODE_MAP_STRUCK]) == []


def test_orphan_drawn_in_its_as_built_box_is_a_finding():
    # The dashed rule overrides the double rule; the word ORPHANED inside a hub
    # box does not, which is the drawing the sketch that settled this got wrong.
    findings = run_checks(_load("node_map_asif_orphan_solid_box.md"), "node-map", NODE_MAP_ALL)
    assert len(findings) == 1
    assert findings[0].check_id == NODE_MAP_DASHED
    assert "Redfang Castle" in findings[0].actual


def test_orphan_named_beside_another_box_does_not_count_as_drawn():
    # The name has to sit inside the dashed rules, not in the prose hanging off
    # the end of a neighbour's box.
    artifact = _load("node_map_asif_orphan_solid_box.md").replace(
        "╎ HALGRIM DEEPDELVER                ╎  held inside the castle",
        "╎ HALGRIM DEEPDELVER                ╎  held inside Redfang Castle",
    )
    findings = run_checks(artifact, "node-map", [NODE_MAP_DASHED])
    assert len(findings) == 1
    assert "Redfang Castle" in findings[0].actual


def test_comma_separated_verdict_still_grades_its_arithmetic():
    # The skill specifies `·`; a page that writes commas gets a verdict on its
    # numbers rather than a numbers finding about its punctuation.
    artifact = _load(ASIF_ORPHANS).replace(
        "Orphaned as-built: Redfang Castle · Halgrim Deepdelver · Stonesong Cave",
        "Orphaned as-built: Redfang Castle, Halgrim Deepdelver, Stonesong Cave",
    )
    assert run_checks(artifact, "node-map", [NODE_MAP_ORPHANS]) == []


def test_counterfactual_rows_are_owned_by_node_map():
    with pytest.raises(ValueError, match="owned by 'node-map'"):
        run_checks(_load(ASIF_ORPHANS), "build-session", [NODE_MAP_ORPHANS])
