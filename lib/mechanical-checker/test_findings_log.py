...
def test_first_append_creates_the_parent_directory(log_path):
    ...
    assert log_finding("build-session", "combat-generator/enemies-line-arithmetic", "mechanical", "healed",
...
def test_finding_record_carries_every_schema_field(log_path):
    log_finding("build-session", "combat-generator/stat-block-refs-on-enemies-line", "mechanical", "unhealable",
...
def test_healed_findings_are_logged_not_filtered(log_path):
    ...
    for _ in range(4):
        log_finding("build-session", "combat-generator/enemies-line-arithmetic", "mechanical", "healed", heal_attempts=1, path=log_path)
...
def test_mechanical_finding_needs_no_evidence_fields(log_path):
    ...
    log_finding("build-session", "combat-generator/budget-line-arithmetic", "mechanical", "healed",
...
def test_the_announcement_carries_the_record_it_lost(tmp_path, capsys):
    ...
    blocked = tmp_path / "a-file-not-a-dir"
    blocked.write_text("", encoding="utf-8")
    log_finding("build-session", "combat-generator/enemies-line-arithmetic", "mechanical", "unhealable",
