...
def test_appends_accumulate_one_json_object_per_line(log_path):
    log_run("combat-generator", ["combat-generator/budget-line-arithmetic", "combat-generator/distinct-stat-block-cap"], path=log_path)
    log_finding("combat-generator", "combat-generator/budget-line-arithmetic", "mechanical", "healed", heal_attempts=1, path=log_path)
    log_finding("combat-generator", "combat-generator/distinct-stat-block-cap", "mechanical", "unhealable", heal_attempts=3, path=log_path)

    records = _lines(log_path)
    assert [r["record"] for r in records] == ["run", "finding", "finding"]
    assert [r.get("inventory_row") for r in records] == [None, "combat-generator/budget-line-arithmetic", "combat-generator/distinct-stat-block-cap"]


def test_appending_never_rewrites_earlier_lines(log_path):
    log_finding("combat-generator", "combat-generator/per-char-matches-budget-table", "mechanical", "healed", heal_attempts=2, path=log_path)
    first = log_path.read_text(encoding="utf-8")
    log_finding("combat-generator", "combat-generator/spotlight-texture-in-palette", "mechanical", "healed", heal_attempts=1, path=log_path)
...
def test_run_record_names_its_tier_and_defaults_to_mechanical(log_path):
    ...
    log_run("combat-generator", ["combat-generator/distinct-stat-block-cap"], path=log_path)
    record, = _lines(log_path)
...
def test_mechanical_run_with_a_verdict_raises(log_path):
    ...
    with pytest.raises(ValueError, match="mechanical run carries no verdict"):
        log_run("combat-generator", ["combat-generator/distinct-stat-block-cap"], verdict="approve", path=log_path)
    assert not log_path.exists()
...
def test_unknown_tier_on_a_run_raises(log_path):
    ...
    with pytest.raises(ValueError, match="unknown tier"):
        log_run("combat-generator", ["combat-generator/distinct-stat-block-cap"], tier="spec", path=log_path)
    assert not log_path.exists()
...
def test_io_failure_is_announced_on_stderr(tmp_path, capsys):
    ...
    assert log_finding("combat-generator", "combat-generator/distinct-stat-block-cap", "mechanical", "healed",
...
def test_every_lost_line_is_announced_not_just_the_first(tmp_path, capsys):
    ...
    for _ in range(3):
        log_finding("combat-generator", "combat-generator/distinct-stat-block-cap", "mechanical", "healed", path=target)
...
def test_default_path_declines_a_working_directory_with_no_campaign_dir(tmp_path, monkeypatch, capsys):
    ...
    assert log_run("combat-generator", ["combat-generator/distinct-stat-block-cap"]) is False
...
def test_default_path_writes_when_the_campaign_dir_is_already_there(tmp_path, monkeypatch):
    ...
    assert log_run("combat-generator", ["combat-generator/distinct-stat-block-cap"]) is True
...
def test_an_explicit_path_is_taken_at_its_word(tmp_path, monkeypatch):
    ...
    assert log_run("combat-generator", ["combat-generator/distinct-stat-block-cap"], path=elsewhere) is True
...
def test_unknown_tier_raises(log_path):
    with pytest.raises(ValueError, match="unknown tier"):
        log_finding("combat-generator", "combat-generator/budget-line-arithmetic", "deterministic", "healed", path=log_path)
    assert not log_path.exists()
...
def test_io_failure_is_swallowed_and_reported(tmp_path):
    ...
    blocked = tmp_path / "a-file-not-a-dir"
    blocked.write_text("", encoding="utf-8")
    assert log_finding("combat-generator", "combat-generator/budget-line-arithmetic", "mechanical", "healed",
                       path=blocked / "findings.jsonl") is False
