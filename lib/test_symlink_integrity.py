...
One library ships this way — the ``wiki-scaffold``
...
The deterministic checker used to be the second row here. It stopped being a
``lib/`` library when the generator fold left it with one consumer: it now lives
where it ships, at ``skills/build-session/scripts/mechanical_checker/``, as a
real directory with no indirection for this guard to check.
