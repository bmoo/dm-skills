...
- **Evidence** lives in the campaign record's structured combat data — the
  encounter ledger and per-session JSONs, wherever the repo's guide says they
  live. **Only this campaign's record feeds it** — never another campaign's
  data, even for the same players. The **variety check** reads it
